...
BUILD_TAG = "rpp_node_pp_csv r7_adaptive_speed"
...
class WaypointFollower(Node):
    def __init__(self) -> None:
        ...
        # NUEVOS: Parámetros para velocidad adaptativa en curvas
        self.declare_parameter("adaptive_speed", True)
        self.declare_parameter("v_min", 0.8)  # Velocidad mínima en curvas cerradas
        self.declare_parameter("curvature_lookahead", 3.0)  # Distancia para calcular curvatura
        self.declare_parameter("curve_detection_points", 10)  # Puntos para análisis de curvatura
        self.declare_parameter("speed_reduction_gain", 1.5)  # Ganancia para reducción de velocidad
        ...
        # NUEVOS: Leer parámetros de velocidad adaptativa
        ...
        self.get_logger().info(
            ...
            f"localization={loc_mode} | speed={speed_mode} | auto={'ON' if self.autonomous_active else 'OFF'}"
        )
    ...
    def _joy_cb(self, msg: Joy):
        ...
        # Botón X: SIEMPRE detiene la navegación (botón de emergencia)
        ...
        # Botón R1: Toggle de activación/desactivación
    ...
    # NUEVA FUNCIÓN: Calcular curvatura local del trayecto
    def _calculate_path_curvature(self, idx: int) -> float:
        """
        Calcula la curvatura promedio del camino adelante del índice actual.
        Curvatura más alta = curva más cerrada
        """
        if len(self.waypoints) < 3:
            return 0.0
        ...
        for k in range(len(points) - 2):
            ...
            # Diferencia de ángulo (normalizada entre -π y π)
        ...
        # Curvatura promedio (en radianes por unidad de distancia aproximada)
        if not angles:
            return 0.0
    ...
    # NUEVA FUNCIÓN: Calcular velocidad objetivo basada en curvatura
    def _calculate_adaptive_speed(self, curvature: float) -> float:
        """
        Calcula la velocidad objetivo basada en la curvatura del camino.
        Curvatura alta -> velocidad baja
        Curvatura baja -> velocidad alta
        """
        if not self.adaptive_speed:
            return self.v_des
        
        # Normalizar curvatura (ajustar estos valores según tu trayecto)
        # Curva típica de 90° en ~5 metros tiene curvatura ~0.3 rad
        curvature_threshold = 0.5  # Umbral para considerar curva cerrada
        
        if curvature < 0.05:  # Recta
            return self.v_des
        
        # Mapeo exponencial: curvas cerradas reducen velocidad más agresivamente
        speed_factor = math.exp(-self.speed_reduction_gain * curvature / curvature_threshold)
        speed_factor = clamp(speed_factor, 0.0, 1.0)
        
        # Interpolar entre v_min y v_des
        target_speed = self.v_min + (self.v_des - self.v_min) * speed_factor
        
        return clamp(target_speed, self.v_min, self.v_des)
    ...
    def _on_timer(self):
        ...
        if self.use_scan and self.avoid_enabled and self.scan_has_data and (self.last_scan is not None):
            ...
            if e_idx > s_idx:
                valid = [r for r in msg_scan.ranges[s_idx:e_idx+1] if not (math.isinf(r) or math.isnan(r))]
            ...
            if dmin < self.danger_dist:
                ...
                if gap is not None:
                    gs, ge = gap
                    xt_gap, yt_gap = self._gap_midpoint_target(msg_scan, gs, ge, self.Ld)
