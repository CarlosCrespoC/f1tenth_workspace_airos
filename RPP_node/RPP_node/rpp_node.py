...
import math, csv
# ... (todos tus otros imports van aquí sin cambios) ...
...
BUILD_TAG = "rpp_node_pp_csv r4"  # <-- AÑADE ESTA LÍNEA
...
class WaypointFollower(Node):
    def __init__(self) -> None:
        ...
        # -------- Parámetros (Declaración sin cambios) --------
        self.declare_parameter("topic_odom", "/odom")
        self.declare_parameter("topic_cmd",  "/drive")
        # ... (todas tus otras declaraciones de parámetros van aquí) ...
        ...
        self.declare_parameter("v_cmd", 7.0)
        self.declare_parameter("steer_limit", 0.6)
        ...
        self.declare_parameter("actual_path_frame", "")
                ...
                # -------- Lee parámetros (Método Corregido y más Seguro) --------
        param_gv = self.get_parameter
        self.topic_odom: str = param_gv("topic_odom").get_parameter_value().string_value
    ...
    def _publish_avoidance_marker(self, xt: float, yt: float):
        """Publica un marcador del objetivo temporal (gap) para RViz."""
    ...
    def _odom_cb(self, msg: PoseWithCovarianceStamped):
        ...
        # Guardar la pose de odometría en el buffer para dibujar la trayectoria real
        if self.publish_actual_path:
            ps = PoseStamped()
            # Usa el mismo header de la odometría (frame y tiempo) para evitar problemas de TF
    ...
    def _on_timer(self):
        ...
        if self.use_scan and self.avoid_enabled and self.scan_has_data and (self.last_scan is not None):
            ...
            # distancia mínima en sector frontal
            if e_idx > s_idx:
            ...
            if dmin < self.danger_dist:
                ...
                if gap is not None:
                    ...
                    # mezcla del objetivo reactivo con el global
        ...
        if used_gap:
            ...
            # borra marcador anterior si no se usa gap esta iteración
        ...
        # Publicar trayectoria real (roja en RViz)
        if self.publish_actual_path and len(self.actual_path) > 1:
            pmsg = PathMsg()
            # frame: si el usuario define uno, úsalo; si no, respeta el de la odometría
            if self.actual_path_frame:
                pmsg.header.frame_id = self.actual_path_frame
            else:
                # tomar el frame del último pose guardado (típicamente "odom" o "map" según tu stack)
                pmsg.header.frame_id = self.actual_path[-1].header.frame_id
